...
const Evidence = ({
  ...
  const getSizeStyles = () => {
    switch (size) {
      case 'small':
        return { containerWidth: wp('15%'), containerHeight: wp('15%'), borderRadius: wp('3%'), padding: wp('0.6%') };
      case 'smallMedium':
        return { containerWidth: wp('18.5%'), containerHeight: wp('18.5%'), borderRadius: wp('3.5%'), padding: wp('0.8%') };
      case 'large':
        return { containerWidth: wp('25%'), containerHeight: wp('25%'), borderRadius: wp('5%'), padding: wp('0.8%') };
      default:
        return { containerWidth: wp('20%'), containerHeight: wp('20%'), borderRadius: wp('4%'), padding: wp('1.2%') };
    }
  };
  ...
  return (
    ...
    >
      <View style={styles.imageContainer}>
        ...
        <Image
          source={imageUri && !imageError ? { uri: imageUri } : require('../../assets/images/profesor/foto.png')}
          style={[
            styles.evidenceImage,
            {
              width: sizeStyles.containerWidth - (containerPadding * 2) - 4,
        ...
        <View style={[styles.overlay, { borderRadius: containerBorderRadius - containerPadding }]} />
      </View>
    </Pressable>
  );
};
...
const styles = StyleSheet.create({
  ...
  imageContainer: { flex: 1, position: 'relative', width: '100%', height: '100%' },
  evidenceImage: { width: '100%', height: '100%' },
  errorImage: { opacity: 0.5 },
  loadingContainer: {
  ...
  badge: {
    ...
    backgroundColor: '#ff6b6b', borderRadius: wp('3%'),
    minWidth: wp('6%'), height: wp('6%'),
    alignItems: 'center', justifyContent: 'center',
    borderWidth: 2, borderColor: 'white',
  },
  badgeText: { color: 'white', fontSize: wp('2.5%'), fontWeight: 'bold' },
